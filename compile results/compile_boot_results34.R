...
###### Input directories 
rm(list=ls())
indir='N:/COVID-19/ACTT/Combined ACTT Analyses/ACTT123 comparison of RDV arm/output/cluster results ACTT 3 vs 4'
...
for (s in 1:length(seeds)){
  load(paste(indir,'/boothrs_both_50imputations_20_resamples_', seeds[s],sep=''))
...
fit2=coxph(Surv(tte_recov, recovered) ~ comparison.group.flag+ cluster(USUBJID), 
           data = dat34_with_weights, 
           weights=dat34_with_weights$wt)
est=fit2$coef
...
## score CI 
score.results = read_sas(paste(outdir, 'scoreci_recovery34.sas7bdat', sep='/'))
...
score.results = read_sas(paste(outdir, 'scoreci_mortality34.sas7bdat', sep='/'))
ci.low = as.numeric(score.results[2,4])
ci.up = as.numeric(score.results[2,5])
...
rownames(tab)=c("Unweighted recovery", "Weighted recovery", "Unweighted mortality",
                "Weighted mortality")
tab
save(tab, file=paste(outdir,'/actt34hrtab',sep=''))
...
dat%>%
  ...
  rtf_encode() %>%
  write_rtf(paste(outdir,"/results34.rtf",sep=''))
