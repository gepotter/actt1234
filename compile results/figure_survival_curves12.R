...
# R version 4.0.4
# Input: dat12_with_weights, actt12hrtab
...
load(paste(outdir,'/actt12hrtab',sep=''))
...
recov=get.curves(tte=dat12$tte_recov, cnsr=(1-dat12$recovered), phase=dat12$phase, 
                wt=dat12$wt)

mort=get.curves(tte=dat12$tte_death, cnsr=(1-dat12$died), 
                phase=dat12$phase, wt=dat12$wt)
...
p.recov.both = ggplot(data=recov, aes(x=days, y=1-survival, 
  ...
  ggtitle('Time to recovery in ACTT-1 and ACTT-2')
...
p.mort.both = ggplot(data=mort, aes(x=days, y=1-survival, 
  ...
  ggtitle('Time to death in ACTT-1 and ACTT-2')
...
p.recov.both = ggplot(data=recov, aes(x=days, y=1-survival, 
  ...
  theme(legend.position='none',
        text=element_text(family='serif'))+
  geom_text(x=14, y=.5, label='ACTT-1', col='navyblue', family='serif')+
  geom_text(x=10, y=.75, label='ACTT-2', col='red', family='serif')+
  scale_colour_manual(values=c("navyblue", "red"))+xlab("Study Day")+
  scale_linetype_manual(values=c("dotted", "solid"))+
  scale_x_continuous(breaks=c(0,7,14,21,28), limits = c(0, 28))+
  ylab('Probability of recovery')+
...
estci= format(as.numeric(tab[3,1:3], nsmall=2))
...
p.mort.both = ggplot(data=mort, aes(x=days, y=1-survival, 
  ...
  scale_x_continuous(breaks=c(0,7,14,21,28), limits = c(0, 28))+
  ylab('Probability of death')+
  geom_text(x=8, y=.06, label='ACTT-1', col='navyblue', family='serif')+
  geom_text(x=14, y=.017, label='ACTT-2', col='red', family='serif')+
...
tab.recov=rbind(c("Model", "Hazard ratio", "95% CI"),
                hr.recov.unadj[1:3],hr.recov.adj[1:3])
tab.recov
...
tab.mort=rbind(c("Model", "Hazard ratio", "95% CI"),
               hr.mort.unadj[1:3],hr.mort.adj[1:3])
...
png("ipw_curves12_both2.png", width=1.3*res*480/72, height=.8*res*480/72, res=300)
