...
# R version 4.0.4
# Input: dat34_with_weights, actt34hrtab
...
load(paste(outdir,'/dat34_with_weights', sep=''))
load(paste(outdir,'/actt34hrtab',sep=''))

dat34 = dat34_with_weights 
...
recov=get.curves(tte=dat34$tte_recov, cnsr=(1-dat34$recovered), phase=dat34$phase, 
                wt=dat34$wt)
mort=get.curves(tte=dat34$tte_death, cnsr=(1-dat34$died), 
                phase=dat34$phase, wt=dat34$wt)
...
# res=300
# setwd(outdir)
# png("ipw_curves34_both.png", width=1.2*res*480/72, height=.6*res*480/72, res=300)
...
p.recov.both = ggplot(data=recov, aes(x=days, y=1-survival, 
  ...
  geom_text(x=14, y=.7, label='ACTT-3', col='navyblue', family='serif')+
  geom_text(x=7, y=.85, label='ACTT-4', col='red', family='serif')+
  scale_colour_manual(values=c("navyblue", "red"))+xlab("Study Day")+
  scale_linetype_manual(values=c("dotted", "solid"))+
  scale_x_continuous(breaks=c(0,7,14,21,28), limits = c(0, 28))+
  ylab('Probability of recovery')+
...
p.mort.both = ggplot(data=mort, aes(x=days, y=1-survival, 
  ...
  scale_x_continuous(breaks=c(0,7,14,21,28), limits = c(0, 28))+
  ylab('Probability of death')+
  geom_text(x=8, y=.035, label='ACTT-3', col='navyblue', family='serif')+
  geom_text(x=14, y=.0, label='ACTT-4', col='red', family='serif')+
...
res=300
setwd(outdir)
png("ipw_curves34_both_slides.png", width=1.3*res*480/72, height=.8*res*480/72, res=300)
